...
) : IFormsFieldsService
{
    ...
    )
    {
        ...
        if (researchAreaDescriptionDto.AreaId is null && !string.IsNullOrEmpty(researchAreaDescriptionDto.DifferentName))
        {
        ...
        var newResearchAreaDescription = mapper.Map<ResearchAreaDescription>(researchAreaDescriptionDto);
        var oldResearchAreaDescription =
            Find(newResearchAreaDescription, researchAreaDescriptionsInMemory)
            ?? await researchAreaDescriptionsRepository.Get(newResearchAreaDescription, cancellationToken);
